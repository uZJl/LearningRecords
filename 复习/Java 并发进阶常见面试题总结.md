...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3hHibpUazd5Ndeu2EYnruH0kLibcMN3AtKztHeibJhnpGMYmA8uG5h5mXQ/640?wx_fmt=png)synchronized关键字原理
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk32OgYNXVtfymH1Xrq47lBT8X2kDk0kqsdnCvvE40GiaZ14VqdNKYE0Jw/640?wx_fmt=png)synchronized关键字原理
...
![img](https://mmbiz.qpic.cn/mmbiz_jpg/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3ZxOdwaakkBrWaMyzvcC7WibyzVQkjVMRAKdqbrXiaAT58810ljEEkMoQ/640?wx_fmt=jpeg)数据不一致

要解决这个问题，就需要把变量声明为**volatile**，这就指示 JVM，这个变量是不稳定的，每次使用它都到主存中进行读取。

说白了， **volatile** 关键字的主要作用就是保证变量的可见性然后还有一个作用是防止指令重排序。

![img](https://mmbiz.qpic.cn/mmbiz_jpg/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk34Gg4leSyWHLAaF0J2XEtGibpOVUv1bu0P3vGCiasgSuTZfGw8kbzaSicw/640?wx_fmt=jpeg)volatile关键字的可见性
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3B8Py1Vy6V3RP3bmCJOQtAbicnAkA3q9Q2bfg8ibQhHvfYhBNQXEiaHsvg/640?wx_fmt=png)ThreadLocal内部类
...
**方式一：通过构造方法实现**![img](https://mmbiz.qpic.cn/mmbiz_jpg/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3jlpiciayhX9Aq3NsrRQYkF5wXUzy2Xlq9pp4AqXb5GjCibfhGnWf9mggQ/640?wx_fmt=jpeg)**方式二：通过Executor 框架的工具类Executors来实现**我们可以创建三种类型的ThreadPoolExecutor：

- **FixedThreadPool** ：该方法返回一个固定线程数量的线程池。该线程池中的线程数量始终不变。当有一个新的任务提交时，线程池中若有空闲线程，则立即执行。若没有，则新的任务会被暂存在一个任务队列中，待有线程空闲时，便处理在任务队列中的任务。
- **SingleThreadExecutor：** 方法返回一个只有一个线程的线程池。若多余一个任务被提交到该线程池，任务会被保存在一个任务队列中，待线程空闲，按先入先出的顺序执行队列中的任务。
- **CachedThreadPool：** 该方法返回一个可根据实际情况调整线程数量的线程池。线程池的线程数量不确定，但若有空闲线程可以复用，则会优先使用可复用的线程。若所有线程均在工作，又有新的任务提交，则会创建新的线程处理任务。所有线程在当前任务执行完毕后，将返回线程池进行复用。

对应Executors工具类中的方法如图所示：![img](https://mmbiz.qpic.cn/mmbiz_jpg/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk31ibmg3dGrXWIsYk01VMnxqLmIV3IJWFTP7ApWMaTiaetdlc9DOJq54icA/640?wx_fmt=jpeg)
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk351a1W1pqAAwOwqeSlVSWmldhSAZblibwwjvbZzlhubT1N0VIOWHFwpg/640?wx_fmt=png)图解线程池实现原理
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk36Q8MLKkickRGk7UZmsG7DkHaf13p66wdu0VRfjeVOfN4oerkdY1crGg/640?wx_fmt=png)JUC原子类概览
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3z4z5dOuDvNC8ob3cz8yaEYNHSdRKPPmJSkVMSDZIkgKHZpogXeKxMg/640?wx_fmt=png)AQS类
...
![img](https://mmbiz.qpic.cn/mmbiz_png/iaIdQfEric9Tw4cBiaKsapdibPjGUTds5rk3zWlHmg36OQG7Q59k0mZKa6f2vq3RqI4LMqGPHBPkleR9aLREZp9BNA/640?wx_fmt=png)AQS原理图

AQS使用一个int成员变量来表示同步状态，通过内置的FIFO队列来完成获取资源线程的排队工作。AQS使用CAS对该同步状态进行原子操作实现对其值的修改。

```
privatevolatileint state;//共享变量，使用volatile修饰保证线程可见性
```
